  ...
  <div>
    ...
    <div>
      ...
      <div style="text-align: left">
        <button @click="select_all(order_list)">全选/取消</button><button   @click="remove_order(order_list)">移除</button>
      </div>
      </div>
    <div style="text-align: left; margin-bottom: 10px;" v-for="(order,index) in order_list" :key="index">
      <div style="background: paleturquoise">
        <input v-model="order.is_order_selected" type="checkbox">
      ...
      methods: {
        remove_order(order_list){
           let order_number_list= []
             ...
             let url = pGlobal.DJANGO_SERVER_BASE_URL+"/nahuo/removeOrders/"
      ...
      on_order_filter_load(args_and){
           // let query_data ;
               let market_name = null;
                ...
                if(args_and.args_and.market_floor === '666'){
                    args_and.args_and.market_floor = ''
                }
