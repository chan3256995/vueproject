<template>
  <div>
      ...
      <label style="color: red;">只导出登录账号跟进的状态 ( 已付款 标签打印 明日有货) 的商品  </label>
      <button @click="out_put_order_for_excel({'for':'for_tag'})" :disabled="submit_btn_disable">导出标签打印订单</button>
        ...
        <div>
         上一次生成记录下载 <a style="margin-left: 1em" v-if="last_time_excel_url!==''" :href="last_time_excel_url">下载</a>
        </div>
      </div>
      ...
      <div style="width: 90% ; padding-bottom: 5px; border: 1px black solid">
        ...
        <button @click="out_put_order_for_excel({'for':'for_logistics_chuanmei'})" :disabled="submit_btn_disable">导出快递打印地址（传美打印格式）</button>
        <a style="margin-left: 1em" v-if="down_logistics_excel_chuanmei_url!==''" :href="down_logistics_excel_chuanmei_url">下载</a>
      </div>


          <div v-if="false" style="width: 90% ; padding-bottom: 5px; border: 1px black solid">
        <label style="color: red; margin-bottom: 5px">只导出登录账号跟进的 状态 (已拿货 快递打印) 的订单  </label>
    ...
    export default {
      ...
      data(){
          return {
            ...
            down_logistics_excel_chuanmei_url:"",
           ...
           out_put_order_for_excel(params){
            ...
            axios.post(url,params).then((res)=>{
             if(res.data.code === "1000"){
               ...
               }else if (params.for === "for_logistics"){
                 this.down_logistics_excel_url = res.data.excel_url
               }else if (params.for === "for_logistics_chuanmei"){
                 this.down_logistics_excel_chuanmei_url = res.data.excel_url
               }
