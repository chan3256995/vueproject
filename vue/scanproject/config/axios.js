...
　　// 在发送请求之前做些什么
        ...
        if(config.url.search("access_token_bk")=== -1){
          ...
          let token = cookiesUtils.getCookies("access_token_bk")
          if(token===""){
            token = null
          }
          config.url = config.url+"?access_token_bk="+token
          // config.url = config.url+"?access_token_bk="+cookiesUtils.getCookies("access_token_bk")
        }
